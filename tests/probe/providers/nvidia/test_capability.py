...
CAPABILITIES = st.builds(ComputeCapability, major=st.integers(0, 20), minor=st.integers(0, 15))


@given(left=CAPABILITIES, right=CAPABILITIES)
...
def test_a_capability_renders_a_dotted_and_a_dot_free_form_and_orders_as_a_pair(
    ...
    """The `sm_NN` target is the dotted version with the dot removed, and comparison follows the
    pair rather than a decimal read, so 9.0 outranks 8.10 instead of losing to it."""
...
def test_the_architecture_table_checks_the_exact_pair_before_the_major(
    ...
    """Ada and Turing share a major with Ampere and Volta, so the exact pair is looked up first
    and everything else maps by major, down to `Unknown` for a generation with no entry."""
