...
from mainboard.probe import CgroupMemory, Host
...
def test_v2_ancestor_walk_takes_the_tightest_finite(
    ...
    (job / "memory.max").write_text(str(100 * _GIB))  # the jobid scope carries the real cap
    ...
    assert cgroup.limit_bytes == 100 * _GIB
...
def test_v1_reads_memsw_on_an_ancestor_with_leaf_unset(
    ...
    (job / "memory.limit_in_bytes").write_text(str(120 * _GIB))
    (job / "memory.memsw.limit_in_bytes").write_text(str(110 * _GIB))
    (leaf / "memory.limit_in_bytes").write_text(_UNLIMITED_V1)
    (leaf / "memory.memsw.limit_in_bytes").write_text(_UNLIMITED_V1)
    point_proc(
    ...
    cgroup = CgroupMemory.probe()
    assert cgroup.capped is True
    assert cgroup.limit_bytes == 110 * _GIB  # memsw is tighter than the RAM limit


def test_uncapped_host_falls_back_to_total_ram(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A host with no finite cap reports total RAM and `capped=False`."""
    point_cgroup_root(monkeypatch, tmp_path)
    ...
    monkeypatch.setattr(
        cgroup_mod.psutil, "virtual_memory", lambda: type("VM", (), {"total": 64 * _GIB})()
    )

    cgroup = CgroupMemory.probe()
    assert cgroup.capped is False
    assert cgroup.limit_bytes == 64 * _GIB
...
def test_unreadable_proc_cgroup_is_uncapped(
    ...
    point_cgroup_root(monkeypatch, tmp_path)
    monkeypatch.setattr(cgroup_mod, "_CGROUP_PROC", tmp_path / "absent")  # never created
    monkeypatch.setattr(
        cgroup_mod.psutil, "virtual_memory", lambda: type("VM", (), {"total": 8 * _GIB})()
    )
    assert CgroupMemory.enforced_limit() is None
    assert CgroupMemory.probe().limit_bytes == 8 * _GIB
...
def test_corrupt_cap_file_is_skipped(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    (node / "memory.memsw.limit_in_bytes").write_text(str(7 * _GIB))
    point_proc(monkeypatch, tmp_path, "9:memory:/scope\n")
    assert CgroupMemory.enforced_limit() == 7 * _GIB
