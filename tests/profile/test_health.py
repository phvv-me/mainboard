...
def test_benign_throttle_does_not_fire() -> None:
    ...
    idle_clock = Diagnosis.of(
        meter_with(peak_gpu_gb=10.0), snapshot(gpu_pct=90, is_throttling=False), capacity_gb=80.0
    )
...
def test_host_offload_from_shared_processes() -> None:
    ...
    shared = Diagnosis.of(
        meter_with(peak_gpu_gb=10.0),
        snapshot(processes=(FakeProcess(pid=1, used_bytes=0), FakeProcess(pid=2, used_bytes=0))),
        capacity_gb=80.0,
    )
...
def test_diagnose_on_cpu_only_host_is_healthy() -> None:
    ...
    out = Diagnosis.diagnose(meter_with(peak_gpu_gb=0.0), None)
    assert out == Diagnosis()
    assert out.reason == "healthy"


def test_diagnose_reads_the_live_device() -> None:
    """`diagnose` snapshots the device and reads its capacity straight from the provider."""
    gpu = FakeGPU(
        memory=FakeMemory(total_gb=80.0),
        reading=snapshot(gpu_pct=95),
    )
    out = Diagnosis.diagnose(meter_with(peak_gpu_gb=79.0), gpu)
    assert out.near_oom
