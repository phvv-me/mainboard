...
class _RecordingProfiler:
    """A `Profiler` returning a fixed kernel `Profile`, settling on a device like the real one,
    since the report is scored against the session's device."""
...
def test_profile_runs_the_callable_and_reports_where_its_time_went(
    ...
    """A requested kind the device lacks is named in `unavailable`, not silently omitted."""
    ran: list[int] = []
    synced: list[int] = []
...
def test_a_discovered_device_still_scores_the_bandwidth(monkeypatch: pytest.MonkeyPatch) -> None:
    """With no device named, bandwidth is graded against the discovered card's peak, not zero."""
    monkeypatch.setattr(bottleneck, "tracer", lambda: _NoTracer())
