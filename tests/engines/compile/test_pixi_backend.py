...
def test_windows_keeps_declared_tasks_on_pixis_task_runner(
    fp: FakeProcess,
    pixi: Pixi,
    tool_paths: Mapping[str, str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Task cwd, dependencies and shell syntax remain Pixi's responsibility on Windows."""
    pixi.manifest.write_text(
        """[environments.default]
features = ["paper"]
