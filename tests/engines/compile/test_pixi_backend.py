...
def test_the_windows_activation_cache_refuses_missing_stale_and_invalid_state(
    ...
    """Restricted execution accepts only a current cache with Pixi's two expected containers."""
...
def test_generated_windows_activation_reports_missing_scripts_and_filters_dotenv_lines(
    ...
    """Only complete assignments absent from both inherited and Pixi activation are imported."""
...
def test_a_dotenv_name_is_matched_against_the_activation_the_way_windows_matches_it(
    ...
    """The cached activation is upper-cased and the batch script it stands in for is case-blind.

    A `.env` line spelled `foo=bar` beside a cached `FOO` therefore added a second spelling of
    the same variable to the block handed to CreateProcess, where the real dotenv.bat would have
    skipped it: `if not defined FOO` is already true.
    """
...
def test_caching_windows_activation_is_a_no_op_off_windows(
    pixi: Pixi, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A POSIX host never needs the cache, so asking for it costs nothing and runs nothing."""
    monkeypatch.setattr("platform.system", lambda: "Linux")
    assert pixi.cache_windows_activation("default") is None
    assert not pixi.windows_activation_cache.exists()


def test_the_cached_activation_refuses_to_serve_when_missing_stale_or_malformed(
    pixi: Pixi, tmp_path: Path
) -> None:
    pixi.manifest.parent.mkdir(parents=True, exist_ok=True)
    pixi.manifest.write_text("[workspace]\n", encoding="utf-8")
    with pytest.raises(MissionError, match="not cached"):
        pixi._cached_windows_activation()
    pixi.windows_activation_cache.write_text('{"environment_variables": []}', encoding="utf-8")
    later = pixi.manifest.stat().st_mtime_ns + 10_000_000_000
    os.utime(pixi.windows_activation_cache, ns=(later, later))
    with pytest.raises(MissionError, match="invalid"):
        pixi._cached_windows_activation()
    pixi.manifest.write_text("[workspace]\nname = 'moved'\n", encoding="utf-8")
    # A whole second newer, since NTFS keeps a modification time only to 100 ns.
    os.utime(pixi.manifest, ns=(later + 1_000_000_000, later + 1_000_000_000))
    with pytest.raises(MissionError, match="activation changed"):
        pixi._cached_windows_activation()


def test_a_generated_dotenv_script_tolerates_a_missing_file_and_skips_non_assignments(
    tmp_path: Path,
) -> None:
    exported: dict[str, str] = {}
    with pytest.raises(MissionError, match="does not exist"):
        Pixi._apply_generated_activation(tmp_path / "absent.bat", exported, set())
    script = tmp_path / "dotenv.bat"
    script.write_text(
        """@echo off
rem Generated by mainboard's Provisioner (workspace.dotenv = true).
if not exist "values.env" goto :eof
""",
    ...
    Pixi._apply_generated_activation(script, exported, set())
    assert exported == {}
    (tmp_path / "values.env").write_text("# a comment\nNOT AN ASSIGNMENT\nLANDED=1\n")
    Pixi._apply_generated_activation(script, exported, set())
    assert exported == {"LANDED": "1"}


def test_a_direct_windows_environment_clears_what_the_unset_script_names(pixi: Pixi) -> None:
    pixi.manifest.parent.mkdir(parents=True, exist_ok=True)
    pixi.manifest.write_text("[workspace]\n", encoding="utf-8")
    script = pixi.manifest.parent / "unset.bat"
    script.write_text(
        "@echo off\nrem Generated by mainboard from the [env] table.\nset TO_CLEAR=\n"
    )
    pixi.windows_activation_cache.write_text(
    ...
    with local.env(TO_CLEAR="inherited"), pixi.direct_windows_environment("default"):
        assert "TO_CLEAR" not in local.env


def test_a_restricted_windows_capture_refuses_an_environment_never_installed(
    pixi: Pixi, monkeypatch: pytest.MonkeyPatch
) -> None:
    pixi.manifest.parent.mkdir(parents=True, exist_ok=True)
    pixi.manifest.write_text("[workspace]\n", encoding="utf-8")
    ...
    monkeypatch.setattr("platform.system", lambda: "Windows")
    with pytest.raises(MissionError, match="not installed"):
        pixi.capture(("tectonic", "--help"), "default")
...
def test_a_restricted_windows_capture_of_a_bare_command_runs_it_from_the_prefix(
    ...
    pixi.windows_activation_cache.write_text(
        json.dumps(
            {"environment_variables": {"Path": str(local.env["PATH"])}, "activation_scripts": []}
        ),
        encoding="utf-8",
    )
    monkeypatch.setattr("platform.system", lambda: "Windows")
...
def test_run_exports_reach_the_child_through_pixis_runner_and_not_the_parent(
    ...
    """Pixi's task shell understands `env NAME=value cmd`, so the exports travel as argv."""
    monkeypatch.setattr("platform.system", lambda: "Linux")
    fp.register([fp.any()])
...
def test_a_failed_lock_or_sync_is_raised_rather_than_run_in(
    ...
    """Neither leaves a caller to run a command in an environment pixi could not settle."""
...
def test_sync_brings_the_prefix_in_line_with_the_lock_without_solving(
    ...
    """A wave of jobs sharing one pinned tree updates the prefix once, before any of them runs."""
    fp.register([fp.any()])
    pixi.sync("serving")
    assert list(fp.calls[0]) == [
        tool_paths["pixi"],
