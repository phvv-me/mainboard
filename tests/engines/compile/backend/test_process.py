...
@pytest.mark.parametrize(
    ("system", "lifetime"),
    [
        pytest.param("Windows", {"creationflags": 0x00000200 | 0x00000008}, id="windows"),
        pytest.param("Linux", {"start_new_session": True}, id="posix"),
    ],
)
def test_detached_processes_release_terminal_handles_with_the_platform_lifetime_boundary(
    system: str, lifetime: dict[str, int], monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert calls == [{"stdin": DEVNULL, "stdout": DEVNULL, "stderr": DEVNULL, **lifetime}]
