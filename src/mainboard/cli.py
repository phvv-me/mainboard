...
def build(root: Path | None = None) -> App:
    ...
    def lint_install_hook() -> None:
        ...
        print(GitHook(workspace_root(), hook="pre-commit", command="lint commit").install())
    ...
    def ci_install_hook(package: Path | None = None) -> None:
        """Make every `git push` of the package run `ci --matrix` first, refused on a failure.

        package: a directory inside the package, the working directory when omitted.
        """
        found = Package.found((package or Path.cwd()).resolve())
        print(GitHook(found.root, hook="pre-push", command="ci --matrix").install())
