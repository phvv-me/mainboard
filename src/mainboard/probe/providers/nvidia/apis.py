...
class NvidiaApis:
    ...
    `protocols.py` (the surface this provider actually calls) and bound to the real,
    untyped modules at runtime. `cuda.bindings` (runtime + NVML) is the required,
    ABI-stable layer for device discovery and memory. `cuda.core` is an optional
    convenience layer, and its compiled extensions can fail to load when another library
    (e.g. a torch wheel bundling an older `libstdc++`) is imported first, so the
    provider keeps working without it.
    """
